...
fastify.register(staticFiles, { root: path.join(process.cwd(), 'public'), prefix: '/', wildcard: false });
...
fastify.post('/shorten', async (req, reply) => {
    try {
        ...
        await pool.query(
            'INSERT INTO urls (slug, target_url, type, device) VALUES ($1, $2, $3, $4)', 
            [slug, target, mode, devMode]
        );
...
fastify.get('/d/:slug', async (req, reply) => {
    ...
    const result = await pool.query('SELECT target_url, device FROM urls WHERE slug = $1 AND type = $2', [slug.toLowerCase(), 'deceiver']);
    if (result.rows.length === 0) return reply.sendFile('404.html');
    
    const { target_url, device } = result.rows[0];
    const userAgent = AGENTS[device] || AGENTS.desktop;
    ...
    <body>
        <iframe src="${target_url}" id="viewer"></iframe>
        <script>
            // El servidor ya mando la orden, pero reforzamos con JS si es posible
            console.log("Modo ejecucion: ${device}");
        </script>
    </body>
    </html>`;
